...
ggplot(smmrsd_vg_sales, mapping = aes(x = Year, y = Total_Global_Sales, colour = Publisher, n = 10))
```
...
That looks a little bit more like a plot. We've now got our data, mappings, scales that represent our data variables in 2D space, and finally some geometric objects that represent our data points.

#### Stats

For a data point to be shown on the plot, it can go through a statistical transformation or 'stat'. In our plot so far, we have no statistical transformation - the value in the dataset is the value that's plotted. But we 
Our two layers also share different 'stats' or statistical transformations. For our lines, we have no statistical transformation - the value in the dataset is the value that's plotted. For our regression line however, we use a statistical transformation to map the values in the dataset to the values that are plotted. We're using the "loess" method for this plot, which by default will use a `y ~ x` formula.

#### Layers

Together, the data, mapping, stat and geom components form a **layer**. A plot can be made up of multiple layers. For example, let's show the same data but overlay a regression line with a slightly different mapping:
...
ggplot(smmrsd_vg_sales, mapping = aes(x = Year, y = Total_Global_Sales)) +
  ...
  geom_smooth(method = "loess", formula = y ~ x) +
  scale_x_continuous(name = "Year") +
  scale_y_continuous(name = "Global Sales (Millions)") +
...
Scales, like facets, are not part of the plot's layers. They will be constant across all layers in the plot.
