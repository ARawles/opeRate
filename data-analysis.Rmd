...
Now that you've got your data in the tidy format, you might want to create some new calculated columns. As we've seen before, we create new columns using the `dplyr::mutate()` function. We're going to use the `datasets::UKgas` dataset for this, which shows quarterly gas consumption in the UK between 1960 and 1986:

```{r}
head(datasets::UKgas)
```

First we need to do some quick tidying to get the dataset in the right format:

```{r}
tidy_gas <- tsibble::as_tsibble(
  datasets::UKgas
) %>%
  tidyr::pivot_longer(cols = tidyselect::starts_with("Qtr"),
                      names_to = "Quarter",
                      names_prefix = "(Qtr)",
                      names_transform = list(Quarter = readr::parse_number))
```
